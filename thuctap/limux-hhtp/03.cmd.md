...
- Để là xem được thì ta cần phải tiến hành cài tool
  ...
  ```
  - Tool này chỉ phân biệt được user dùng ssh thôi
- Chỉnh sửa lại file `~/.bashrc` bằng quyền root
  ```
  vi ~/.bashrc
  ```
...
- Lưu ý: tool này chỉ áp dụng với các user đã tạo mới và không áp dụng với user đã tồn tại trước đó nên là một hạn chế .Phù hợp với máy mới 
